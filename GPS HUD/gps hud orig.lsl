...
default
	...
	state_entry()
		{
		llSetText("Activating...", <1.0, 0.0 ,0.0>, 1.0);
	...
	timer()
		...
		if (curr_sim_pos != prev_sim_pos)
			llOwnerSay("Left " + prev_sim_name + ", Entered " + curr_sim_name + ":" + (string)curr_sim_pos);
		...
		string display = "";
		display += "Compass: " + (string)(llRound(math_to_nav(curr_rot.z)))
					+ "  Track: " + (string)(llRound(speed_polar.y)) + "\n";
		display += "Speed: " + fixedPrecision(speed_polar.x, 1) + " knots\n";
//		display += "Alt: " + " AGL: " + fixedPrecision(alt_agl, 1) + " meters "
//					+ " AWL: " + fixedPrecision(alt_awl, 1) + " meters\n";
		display += "Alt: " + " ATL: " + fixedPrecision(alt_atl, 1) + " meters Depth: " + fixedPrecision(wat_dep, 1) + " meters\n";
		display += "ROC: " + fixedPrecision(speed_cart.z, 1) + " mps\n";
		display += "Wind: " + (string)(llRound(wind_polar.y)) + " @ " + fixedPrecision(wind_polar.x, 1);
		...
		if (warn_pos.x > 256.0 || warn_pos.x < 0 || warn_pos.y > 256.0 || warn_pos.y < 0)
			...
			text_color = CAUT_COLOR;
		else
			text_color = OK_COLOR;

		// display text
		llSetText(display, text_color, 1.0);
		...
		if (llGetParcelFlags(warn_pos) & (PARCEL_FLAG_USE_ACCESS_GROUP | PARCEL_FLAG_USE_ACCESS_LIST))
			...
			llSetColor(OK_COLOR, ALL_SIDES);

		// set previous position information
		prev_sim_pos  = curr_sim_pos;
		prev_sim_name = curr_sim_name;
		prev_lcl_pos  = curr_lcl_pos;
		prev_gbl_pos  = curr_gbl_pos;
